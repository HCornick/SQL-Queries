...
staff s INNER JOIN address a ON s.address_id = a.address_id;

-- 6b. 	Use JOIN to display the total amount rung up by each staff member in August of 2005. --
-- 			Use tables staff and payment. --
SELECT s.first_name, s.last_name, concat('$',format(SUM(amount),2)) AS 'total_amount_August_05' FROM 
staff s INNER JOIN payment p ON s.staff_id = p.staff_id 
WHERE p.payment_date LIKE "2005-08%" 
GROUP BY last_name, first_name;

-- 6c. 	List each film and the number of actors who are listed for that film. Use tables film_actor and film. Use inner join. --
SELECT title, count(actor_id) AS 'actor_count' FROM 
film f INNER JOIN film_actor fa ON f.film_id = fa.film_id 
GROUP BY title;
...
SELECT first_name, last_name, concat('$',format(SUM(amount),2)) AS 'total_paid' FROM 
payment p INNER JOIN customer c ON p.customer_id = c.customer_id 
GROUP BY last_name, first_name;
...
SELECT f.title, l.name AS 'language' FROM 
film f INNER JOIN language l ON f.language_id = l.language_id 
WHERE ((title LIKE "K%") OR (title LIKE "Q%")) AND l.language_id IN(
SELECT l.language_id FROM language WHERE name = "English");
...
(((customer cu INNER JOIN address a ON cu.address_id = a.address_id)
...
SELECT f.title, c.name AS 'film_category' FROM 
((film f INNER JOIN film_category fc ON f.film_id = fc.film_id)
INNER JOIN category c ON fc.category_id = c.category_id)
WHERE name = "Family";

-- 7e. 	Display the most frequently rented movies in descending order. --
SELECT title, COUNT(r.rental_id) as 'total_times_rented' FROM 
...
SELECT store.store_id, concat('$',format(SUM(p.amount),2)) AS 'total_business' FROM 
((store INNER JOIN staff ON store.store_id = staff.store_id)
INNER JOIN payment p ON staff.staff_id = p.staff_id)
GROUP BY store_id;

-- 7g. 	Write a query to display for each store its store ID, city, and country. --
SELECT s.store_id, ci.city, co.country FROM 
(((store s INNER JOIN address a ON s.address_id = a.address_id)
INNER JOIN city ci ON a.city_id = ci.city_id)
INNER JOIN country co ON ci.country_id = co.country_id);

-- 7h. 	List the top five genres in gross revenue in descending order. --
-- 			(Hint: you may need to use the following tables: category, film_category, inventory, payment, and rental.) --
SELECT c.name AS 'genre', concat('$',format(SUM(p.amount),2)) AS 'gross_revenue' FROM 
...
CREATE VIEW top_five_genres AS (SELECT c.name AS 'genre', concat('$',format(SUM(p.amount),2)) AS 'gross_revenue' FROM 
((((category c INNER JOIN film_category fc ON c.category_id = fc.category_id)
INNER JOIN inventory i ON fc.film_id = i.film_id)
INNER JOIN rental r ON i.inventory_id = r.inventory_id)
INNER JOIN payment p ON r.rental_id = p.rental_id)
